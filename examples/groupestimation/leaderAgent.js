...
function runLeaderDiagnostic(GameState) {
    ...
    if (needsObstacleClearing){
        ...
        // red and yellow both failed — try an obstacle move before attempting blue
        console.log('🤖 red/yellow failed, trying obstacle move before blue');
        const obstaclePlan = suggestObstaclePlan(affected.length > 0 ? affected : perBlock.filter(e => !snapshot.blocks?.[e.block]?.locked));
        if (obstaclePlan && obstaclePlan.score > 0){
            console.log('🤖 Obstacle plan (fallback):', obstaclePlan);
            return {
                chosen: {
                    type: 'obstacle',
                    id: obstaclePlan.obstacle,
                    dir: obstaclePlan.steps[0]
                }
            };
        }

        // obstacle also failed — try blue last
        console.log('🤖 Obstacle fallback failed, trying blue');
        const blueResult = chooseDownhillStepWithBlockGate("blue");
        if (blueResult.ok){
            console.log('🤖 Block move chosen (blue fallback):', blueResult);
            return {
                chosen: {
                    type: 'block',
                    id: blueResult.block,
